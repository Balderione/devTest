trigger TRIT05_ContractAfterUpdate on Contract (after insert, after update) {
    ...
    if(trigger.isUpdate){
        if(!ControlTrigger && trigger.newMap.get(trigger.new[0].Id).IT_SkipActivation__c == trigger.oldMap.get(trigger.new[0].Id).IT_SkipActivation__c){
            ...
            if( trigger.newMap.get(trigger.new[0].Id).IT_Commission_Type__c != trigger.oldMap.get(trigger.new[0].Id).IT_Commission_Type__c || 
                ...
                trigger.newMap.get(trigger.new[0].Id).IT_Discount_Percentage_2__c != trigger.oldMap.get(trigger.new[0].Id).IT_Discount_Percentage_2__c ||
                trigger.newMap.get(trigger.new[0].Id).IT_Discount_Percentage_3__c != trigger.oldMap.get(trigger.new[0].Id).IT_Discount_Percentage_3__c ||
                trigger.newMap.get(trigger.new[0].Id).IT_Discount_Validity_Date_2__c != trigger.oldMap.get(trigger.new[0].Id).IT_Discount_Validity_Date_2__c ||
                trigger.newMap.get(trigger.new[0].Id).IT_Discount_Validity_Date_3__c != trigger.oldMap.get(trigger.new[0].Id).IT_Discount_Validity_Date_3__c ||
                trigger.newMap.get(trigger.new[0].Id).IT_Extradiscount_Percentage__c != trigger.oldMap.get(trigger.new[0].Id).IT_Extradiscount_Percentage__c ||
                trigger.newMap.get(trigger.new[0].Id).IT_Type_Extradiscount__c != trigger.oldMap.get(trigger.new[0].Id).IT_Type_Extradiscount__c){
    ...
    if(trigger.isInsert){
        if(!ControlTrigger){
            list<Contract> docOk = new list<Contract>();
            for(Contract sinContr : trigger.new){
                if(!String.isBlank(sinContr.IT_Barcode__c))
                    docOk.add(sinContr);
            }
            if(docOk != null && docOk.size() > 0)
                APIT12_CallOutbound.createRequestContract(docOk, 'Document');
        }
    }    
}